...
class LocationInfo(DataTable):
    ...
    def __init__(self, *, id: str = 'location-info', classes: str = 'weather-widget'):
        ...
        self.add_column('Value', key='value', width=26)
        self.add_row(Text('', style='dim'), Text('', style='bold'), key='date')
        self.add_row(Text('', style='dim'), Text('', style='bold'), key='time')
        self.add_row(Text('', style='dim'), Text('', style='bold'), key='tz')
        self.add_row(Text('', style='dim'), Text('', style='bold'), key='coords')
        self.add_row(Text('', style='dim'), Text('', style='bold'), key='elev')
    ...
    def _update_display(self) -> None:
        ...
        date_text, time_text, tz_text = self._build_datetime_displays(data['tz_identifier'], theme)

        # Build coordinate and elevation displays
        coords_text = self._build_coords_display(data['lat_val'], data['lon_val'], theme)
    ...
    def _extract_location_data(self, forecast_metadata: object | None, location: object | None) -> dict:
    ...
    def _build_datetime_displays(self, tz_identifier: str | None, theme: dict) -> tuple[Text, Text, Text]:
        """Build formatted date, time, and timezone displays with theme colors."""
        # Get current time in forecast timezone
        tzinfo = ZoneInfo(tz_identifier) if tz_identifier else datetime.datetime.now().astimezone().tzinfo
        now = datetime.datetime.now(tzinfo)

        # Build date text
        date_text = Text.from_markup(
            f'[bold {theme.get("text-primary")}]{now.strftime("%A ")}[/][{theme.get("text-primary")}]{now.strftime("%d %B %Y")}[/]'
        )
        ...
        time_text = Text.from_markup(
            f'[bold][{theme.get("text-secondary")}]{now.strftime("%H:%M:%S")}[/bold] {tz_abbr} ({offset_str})[/]'
        )

        # Build timezone identifier text
        tz_text = Text.from_markup(f'[bold {theme.get("text-warning")}]{tz_identifier}[/]') if tz_identifier else Text('')
    ...
    def _build_coords_display(self, lat_val: float | None, lon_val: float | None, theme: dict) -> Text:
        """Build formatted coordinate display with theme color."""
        lat = self._fmt_lat(lat_val)
        lon = self._fmt_lon(lon_val)
        if lat_val is not None and lon_val is not None:
            url = f'https://www.openstreetmap.org/#map=12/{lat_val:.5f}/{lon_val:.5f}'
            return Text.from_markup(
                f'[link={url}][italic {theme.get("text-accent")}]{lat}[/], [italic {theme.get("text-accent")}]{lon}[/][/]'
            )
        return Text.from_markup(f'[italic {theme.get("text-accent")}]{lat}[/], [italic {theme.get("text-accent")}]{lon}[/]')

    def _build_elev_display(self, elev_val: float | int | None, theme: dict) -> Text:
        """Build formatted elevation display with theme color."""
        elev = str(int(elev_val)) if isinstance(elev_val, (int, float)) else (str(elev_val) if elev_val else '')
        return Text.from_markup(f'[bold {theme.get("text-error")}]{f"{elev}m" if elev else ""}[/]')
