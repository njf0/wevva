...
class HourlyForecast(Container):
    ...
    def _ensure_valid_active_tab(self, desired_ids: list[str]) -> None:
        ...
        if self._active_tab_id not in id_to_tab:
            self._active_tab_id = next(
                (tid for tid in desired_ids if tid in id_to_tab), None
            )
    ...
    def _build_tab_label_for_date(self, d: datetime.date) -> Text:
        ...
        if d == first_date:
            ...
            return Text.from_markup(
                f"Next 24 Hours [bold {cmin}]{int(tmin)}°[/]-[bold {cmax}]{int(tmax)}°[/]"
            )

        date_str = d.strftime("%A")
        daily_ts = self.daily_model.forecast_timeseries
        row_index = next(
            (i for i, day in enumerate(daily_ts) if day.get("time") == d), None
        )
        if row_index is not None:
            ...
            cmax = temp_colour(tmax, hex=True, unit=temp_unit)
            return Text.from_markup(
                f"""{date_str} [bold {cmin}]{int(tmin)}°[/]-[bold {cmax}]{int(tmax)}°[/]"""
            )
    ...
    def _update_for_date(self, date: Any) -> None:
        ...
        rains = [
            hourly_model.get_precipitation_probability(i) for i in self._col_indices
        ]
    ...
    def _update_emoji_row(self) -> None:
        ...
        for i, abs_idx in enumerate(self._col_indices):
            ...
            style = (
                f"bold {theme_vars.get(cond.color_var)}"
                if (cond and cond.color_var)
                else "bold"
            )
            self.table.update_cell(
                "emoji", col_key, Text(cell, style=style, justify="center")
            )
    ...
    def _update_rain_rows(self, rains: list[float]) -> None:
        ...
        for i, r in enumerate(rains):
            col_key = self._col_keys[i]
            colour = rain_colour(
                r, hex=True, min_colour=theme_vars["foreground"], max_colour=rain_max
            )
            self.table.update_cell(
                "rain_blocks",
                col_key,
    ...
    def _update_wind_rows(self, winds: list[float]) -> None:
        ...
        for i, wv in enumerate(winds):
            col_key = self._col_keys[i]
            colour = wind_colour(
                wv, hex=True, min_colour=theme_vars["foreground"], max_colour=wind_max
            )
            self.table.update_cell(
                "wind_blocks",
                col_key,
    ...
    def _update_footer_row(self) -> None:
        ...
        for i, abs_idx in enumerate(self._col_indices):
            ...
            self.table.update_cell(
                "footer", col_key, Text(hour_label, style="bold", justify="center")
            )
